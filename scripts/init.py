...
if __name__ == '__main__':
    ...
    if '.ros2_workspace' in os.listdir():
        error('This folder is already a workspace root.')
        exit()
    ...
    if 'src' not in os.listdir():
        warn('This directory does not contain a src folder. Are you sure it is a workspace root?')

    if get_workspace_root(os.getcwd()) is not None:
        warn('This folder is already in a ROS 2 workspace. Are you sure you want to continue?')
