...
from tuda_workspace_scripts.print import Colors, print_error, print_info, print_color
...
def print_changes(path):
    ...
    for branch in repo.branches:
        ...
        try:
            ...
            print_error("{} has error on branch {}: {}".format(path, branch.name, e.message))
    ...
    if any(repo.untracked_files) or any(stash) or any(uncommited_commits) or any(local_branches) or any(changes):
        ...
        for branch in local_branches:
            print_color(Colors.LRED, "  Local branch with no remote set up: {}".format(branch))
        for branch in deleted_branches:
            print_color(Colors.LRED, "  Local branch for which remote was deleted: {}".format(branch))
        if any(stash):
        ...
        for item in changes:
            if item.change_type.startswith('M'):
                ...
                print_color(Colors.GREEN, "  Renamed: {} -> {}".format(item.a_path, item.b_path))
                ...
                print_color(Colors.GREEN, "  Copied: {} -> {}".format(item.a_path, item.b_path))
            elif item.change_type.startswith('T'):
                print_color(Colors.ORANGE, "  Type changed: {}".format(item.a_path))
            else:
                print_color(Colors.RED, "  Unhandled change type '{}': {}".format(item.change_type, item.a_path))
        if len(repo.untracked_files) < 10:
            for file in repo.untracked_files:
                print_color(Colors.DGRAY, "  Untracked: {}".format(file))
        else:
            print_color(Colors.DGRAY, "  {} untracked files.".format(len(repo.untracked_files)))
        print("")
    elif repo.is_dirty():
        print_info(path)
